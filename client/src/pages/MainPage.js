...
const MainMenuWrapper = styled.div`
  ...
  grid-template-columns: repeat(4, minmax(194px, auto));
...
const MainMenuCard = styled.div`
  ...
  padding: 1.5rem 2rem;
  box-shadow: 0 0 10px #00baff, 0 0 20px #00baff, 0 0 10px #00baff inset, 0 0 20px #00baff inset, 0 0 30px rgba(0, 186, 255, 0.6) inset;
...
const MainPage = ({ history }) => {
  ...
  const { ssetTableId1 } = useContext(gameContext);
 
  useScrollToTopOnPageLoad();
  const onClickQuickGameButton = () => {
    ssetTableId1(1);
    history.push('/play')
  }
  return (
    ...
    >
      ...
      <MainMenuWrapper>
        ...
        <MainMenuCard onClick={() => onClickQuickGameButton()}>
